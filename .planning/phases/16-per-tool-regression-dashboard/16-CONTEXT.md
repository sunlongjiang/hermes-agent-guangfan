...
- **D-14:** [informational] **不**实现 p25-based hard gate（Pitfall 10 双中门的右侧）。Rationale：当前 holdout per-tool sample_count 通常 < 20，p25 估计方差大，硬 gate 容易误杀；先靠 dashboard warning + 人工 review，待 Phase 14 session 数据 + Phase 19 prompt 数据扩 holdout 体量后再评估。归 deferred。
